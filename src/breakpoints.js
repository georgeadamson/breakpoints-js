...
// Rudimentary detection of display type using breakpoints defined in the CSS. Store the results in h3g.detect object.
// The advantage of this technique is simply that the breakpoints don't have to be defined and duplicated in the JS too.
// Eg: h3g.detect.phone === true
// IMPORTANT: Must be used with detect-display.scss.
// IMPORTANT: Must use with respond.js to use this Media Query technique in IE6-8 etc.
// TODO: Perhaps this could be refactored as a Modernizr plugin?
...
;function( window, undefined ){
  ...
  ;(function (factory) {
    ...
    // Define an object on which to define custom display-size properties: (Eg: breakpoints.phone = true & breakpoints.tablet = false)
    ...
    ;(function( detectBreakpoint ){
      ...
      var $head    = $('HEAD:first')
        , current  = $head.css('fontFamily') || stylesMissing             // Read currently applicable display type: (We rely on this to vary, depending on which mediaquery is being applied right now)
        , names    = $head.children().css('fontFamily').split(/\s*,\s*/)  // Read list of all possible display variants defined in css so we can iterate through them: (See detect-display.scss for definition of these values)
